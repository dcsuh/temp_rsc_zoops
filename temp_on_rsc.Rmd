...
m6b_ll <- function(f, arr_t, h, w){
  ...
  data <- tmp 
  output <- as.data.frame(mapply(m4_sim, R=data$amt_init, time=data$time/60/24, f=f/data$vol,
                                 length=data$mm, gamma=2, arr_t=arr_t, ref_t=15, temp=data$temp, h=h*data$vol + w*data$temp))
  colnames(output) <- "endpoint"
  data$end <- output$endpoint
  data %<>% mutate(resid = log(end) - log(amt_rem))
...
m6b_f_fit <- mle2(m6b_ll,
                skip.hessian = T,
                start=list(f=10, arr_t=50, h=0.01, w=10),
                method = "L-BFGS-B",
                lower = c(0, 0, 0, -Inf),
                upper = c(Inf, Inf, Inf, Inf),
                control=list(parscale=c(f=10, arr_t=50, h=0.01, w=10), maxit=10000))
end_time <- Sys.time()
...
m6c_ll <- function(f, arr_t, h, w){
  ...
  data <- tmp 
  output <- as.data.frame(mapply(m4_sim, R=data$amt_init, time=data$time/60/24, f=f/data$vol,
                                 length=data$mm, gamma=2, arr_t=arr_t, ref_t=15, temp=data$temp, h=h*data$vol*exp(w*data$temp)))
  colnames(output) <- "endpoint"
  data$end <- output$endpoint
  data %<>% mutate(resid = log(end) - log(amt_rem))
