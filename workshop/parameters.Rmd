...
The loop in here isn't working properly
```{r}
...
for(i in 1:length(trts)){
  ...
  beta.summary$treatment[i] <- dsub$ID[1]
...
#### used for beta start####
  ...
  dsub <- dataset[dataset$ID==treatments[j],]
...
### used for S* start###
    ...
    boot.summary$S.boot[k] <- r*(1-((boot.summary$d.boot[k]/boot.summary$b.boot[k])/K))/f
...
boot.summary$R_naught.boot[k] <- (boot.summary$S.boot[k]*beta.list$beta[k]*boot.summary$yield[k])/M
...
### end of bootstrap loop ###
  ...
  lt.summary$S.S.975[j] <- quantile(boot.summary$S.boot, probs=seq(0.025, 0.975, 0.95))[2]  
  lt.summary$S.S.025[j] <- quantile(boot.summary$S.boot, probs=seq(0.025, 0.975, 0.95))[1]

  lt.summary$yield[j] <- boot.summary$yield[k]
...
So scripting this seems to be going pretty well but there are some challenges with bootstrapping everything together because different parameters estimate using different subset of the data.

Beta uses both infected and uninfected individuals because it relies upon prevalence

growth rate (r) treats infected and uninfected individuals separately since we are interested in susceptible host density

So I can run this assuming that infected and uninfected individuals are separate treatments or pool them togther. Alternatively we do bootstraps separately
