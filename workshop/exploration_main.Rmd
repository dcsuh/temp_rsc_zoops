...
Recorded Prevalence
```{r}
mort %<>% filter(!male %in% c(1) & !removed %in% c(1) & !KBP %in% c(1) & !missing %in% c(1)) %>% select(-c(male,removed,KBP,missing))
...
prevalence$ID <- factor(mort$ID, levels = prevOrder)
...
```{r}
lifespan <- mort
lifespan %<>% select(ID, mortality_day, inf)
...
inf_order <- c("15_0.1_D","15_0.5_D","15_1_D",
              "20_0.1_D","20_0.5_D","20_1_D",
              "25_0.1_D","25_0.5_D","25_1_D",
              "15_1_C","20_1_C","25_1_C",
              "2V_1_D","6V_1_D","14V_1_D")
...
```{r}
fitness %<>% filter(!male %in% c(1) & !REMOVED %in% c(1) & !KBP %in% c(1)) %>% select(-c(male,REMOVED,KBP))
fitness %<>% mutate(ID = paste(temp, resource, species,sep = "_"), .before = species) %>% select(-c(tube, temp, resource, species))
...
fit_by_ID <- fitness %>% group_by(ID) %>% summarize(total_fit = sum(fitness, na.rm = T)) # removed NAs
fit_average <- fit_by_ID %>% group_by(ID) %>% 
  summarize(n = n(), mean_fit = mean(total_fit, na.rm = T), var = var(total_fit, na.rm = T), se = sqrt(var(total_fit, na.rm = T)/n())) # remove NAs
fit_average %<>% left_join(.,treatment_factors)
