...
length <- mort %>% group_by(ID) %>% 
  summarize(n=n(), 
            mean_length = mean(length_RAW, na.rm = T), 
            var = var(length_RAW, na.rm = T), 
            se = sqrt(var(length_RAW,na.rm = T)/n()))
...
prevalence$temperature <- factor(prevalence$temperature, levels = c(25, 20, 15))
...
length %>% filter(temperature %in% const_temp & species =="D") %>% 
  ...
  labs(y = "Average Length (um)", x = "Resource Concentration mgC/L", color = "Infection\nStatus" ) +
  theme_bw()
...
```{r}
first_repd <- fitness %>% filter(is.na(male)) %>% pivot_longer(.,8:52)
# first_repd_check <- first_repd %>% group_by(tube) %>%
#   summarize(fitness = sum(value, na.rm = T)) %>%
#   filter(fitness>0) %>%
#   ungroup()

first_repd %<>% group_by(tube, temp, resource, species) %>%
  filter(value>0) %>%
  filter(row_number() == 1) %>%
  ungroup()
