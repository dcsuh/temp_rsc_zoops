...
control_coef <- summary(lm(control_read~resource, data=data))$coef[2]
...
data %<>% mutate(mm = as.numeric(length)*17.86/1000,
                ...
                rate_len = rate/mm^2,
                amt_rem = read/control_coef, #final resource concentration
                amt_rem_tot = amt_rem*vol,
                amt_init = control_read/control_coef) #final resource amount
```
...
data_summ <- data %>% group_by(temp, resource) %>% summarize(rate_mean = mean(rate),
                                                             ...
                                                             time_mean = mean(time_day, na.rm = T))
