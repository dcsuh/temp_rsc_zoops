...
data %<>% mutate(length = as.numeric(length),
                 start_min = (time_in_hr+time_in_min/60)/24,
                 end_min = (time_measured_hr+time_measured_min/60)/24,
                 time = end_min-start_min,
                 vol = 0.015) #volume in L
...
data %<>% filter(trt=="trt") %>% mutate(mm = as.numeric(length)*17.86/1000,
