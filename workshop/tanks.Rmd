...
```{r}
ggplot(summary, aes(x=sample_date,y=mean_total, color = temp)) + geom_point() + geom_line(aes(group=treatment)) + facet_grid(rows= vars(temp), cols = vars(community)) 

ggplot(summary, aes(x=sample_date,y=mean_daphnia, color = temp)) + geom_point() + geom_line(aes(group=treatment)) + facet_grid(rows= vars(temp), cols = vars(community)) 

ggplot(summary, aes(x=sample_date,y=mean_cerio, color = temp)) + geom_point() + geom_line(aes(group=treatment)) + facet_grid(rows= vars(temp), cols = vars(community)) 
```
...
```{r}
data %>% filter(community=="DCM") %>% ggplot(., aes(x=sample_date,y=d_ratio, color = temp)) + geom_point() +geom_line(aes(group = bucket)) + facet_wrap(vars(temp))
```
...
```{r}
size_ratio <- data %>% mutate(j_ratio = juvenile_daphnia_uninf/daphnia)
```


```{r}
ggplot(size_ratio, aes(x=sample_date, y=j_ratio, color = temp)) + geom_point() + geom_line(aes(group=bucket)) + facet_grid(rows= vars(temp), cols = vars(community))
```
