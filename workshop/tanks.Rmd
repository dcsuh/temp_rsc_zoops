...
ggplot(summary, aes(x=sample_date,y=mean_daphnia, color = temp)) + geom_point() + geom_line(aes(group=treatment)) + facet_wrap(vars(community))

ggplot(summary, aes(x=sample_date,y=mean_cerio, color = temp)) + geom_point() + geom_line(aes(group=treatment)) + facet_wrap(vars(community))
```

Juvenile and Adult Ratio
```{r}
size_ratio <- data %>% mutate(j_a_ratio = juvenile_daphnia_uninf/adult_daphnia_uninf)
```


```{r}
ggplot(size_ratio, aes(x=sample_date, y=j_a_ratio, color = community)) + geom_point() + geom_line(aes(group=bucket)) + facet_wrap(vars(temp)) + geom_hline(yintercept=1)

ggplot(size_ratio, aes(x=sample_date, y=j_a_ratio, color = temp)) + geom_point() + geom_line(aes(group=bucket)) + facet_wrap(vars(community)) + geom_hline(yintercept=1)
```
