...
spores %>% ggplot(., aes(x=ID, y=mean_yield)) + 
  ...
  labs(title = "Average Spore Yield", y = "Mean Yield")
spores %>% ggplot(., aes(x=ID, y=log(mean_yield))) + 
  geom_col() + 
  labs(title = "Average Spore Yield log-transformed", y = "log(Mean Yield)")
```
...
ggplot(lifespan,aes(x=ID, y = span)) + geom_boxplot()

ggplot(lifespan,aes(x=ID, y = span)) + geom_boxplot() + 
  stat_summary(geom="text", fun.y=quantile, aes(label=sprintf("%1.1f", ..y..), color=factor(ID)), position=position_nudge(x=0.6), size=3.5)
```
