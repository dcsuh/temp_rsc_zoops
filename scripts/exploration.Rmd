...
prevalence %>% ggplot(.,aes(x=ID,y=prev)) +
  geom_col() + 
  labs(title = "Prevalence") + theme_minimal()
```
...
prev_adj %>% ggplot(.,aes(x=ID,y=prev)) +
  geom_col() + 
  labs(title = "Threshold-adjusted Prevalence") + theme_minimal()
```
...
fit_average %>% ggplot(., aes(x=ID, y=mean_fit)) +
  ...
  labs(title = "Average Total Fitness w/ SE") + theme_minimal()
```
...
spores %>% ggplot(., aes(x=ID, y=mean_yield)) + 
  ...
  labs(title = "Average Spore Yield w/ SE", y = "Mean Yield") + theme_minimal()
spores %>% ggplot(., aes(x=ID, y=log(mean_yield))) + 
  geom_col() + 
  labs(title = "Average Spore Yield log-transformed", y = "log(Mean Yield)") + theme_minimal()
```
...
mean_span %>% ggplot(.,aes(x=ID, y=as.numeric(mean_span))) +
  geom_col() + 
  geom_errorbar(aes(ymin = mean_span-se, ymax = mean_span+se)) +
  geom_text(aes(label=round(as.numeric(mean_span), digits = 2)), vjust = -0.5) +
  labs(title = "Average lifespan with standard error bars", y = "Average lifespan")

lifespan %>% drop_na() %>% ggplot(.,aes(x=ID, y = as.numeric(span))) + geom_boxplot()

lifespan %>% drop_na() %>% ggplot(.,aes(x=ID, y = as.numeric(span))) + geom_boxplot() + 
  stat_summary(geom="text", fun=quantile, aes(label=sprintf("%1.1f", ..y..), color=factor(ID)), position=position_nudge(y=0.35), size=3.5) + theme_minimal() + theme(legend.position="none")
```
