...
```{r, m3a_sim}
m3a_sim <- function(R, time, f, u, length, gamma, Z, h, k, resource){
  times <- seq(0, time, by=0.01)
  xstart <- c(R=R,
              S=0.999,
              I=0,
...
```{r, m3a_ll}
m3a_ll <- function(f, u, h, k){
  data <- tmp
  R_end <- as.data.frame(mapply(m3b_sim, R=data$amt_init*fora_vol/1000, time=data$time/60/24, f=f/fora_vol,
                                 u = 0, #u actually not important for this part. just wanted to use a single deSolve function for both
                                 length=data$mm, gamma=gamma, Z=0,
                                h=h*fora_vol,
                                k=k,
...
#  exp_data <- dataset_tmp %>% filter(exposed==TRUE)
  ...
  I_end <- as.data.frame(mapply(m3b_sim, R=exp_data$resource*life_vol/1000, time=exp_data$time, f=f/life_vol, 
                                u=u, length = exp_data$life_mm, gamma=gamma, Z=spore_conc*life_vol,
                                h=h,
                                k=k,
...
start_time <- Sys.time()
tmp <- data
m3a_u_f_fit <- mle2(m3a_ll, start=list(u = 0.01, f=1, h=1, k=-10), 
                     control=list(parscale = c(u = 0.01, f=1, h=1, k=-10)),
                     skip.hessian=F, method="L-BFGS-B", 
                   lower=c(u = 0, f = 0, h=0, k=-Inf), 
                   upper=c(u = 1, f = Inf, h=Inf, k=Inf))
end_time <- Sys.time()
