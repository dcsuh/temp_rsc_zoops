...
start_time <- Sys.time()
tmp <- data
m3a_u_f_fit <- mle2(m3a_ll, start=list(u = 0.0001, f=1, h=1, k=1), 
                     control=list(parscale = c(u = 0.0001, f=1, h=1, k=1)),
                     skip.hessian=F, method="L-BFGS-B", 
...
```{r, m4_sim}
m4_sim <- function(R, time, f, u, length, gamma, Z, ref_t, arr_t_f, arr_t_u, h){
  times <- seq(0, time, by=0.01)
  xstart <- c(R=R,
              S=1,
              I=0,
