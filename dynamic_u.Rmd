...
m1_ll <- function(f, u){
  data <- tmp
  R_end <- as.data.frame(mapply(m1_sim, R=data$amt_init*0.015, time=data$time/60/24, f=f/data$vol,
  ...
  data %<>% mutate(resid = log(end) - log(amt_rem*0.015))
...
#  exp_data <- dataset %>% filter(exposed==TRUE)
  ...
  I_end <- as.data.frame(mapply(m1_sim, R=exp_data$resource*0.05, time=exp_data$time, f=f/50, u=u, length = exp_data$life_mm, gamma=2, Z=200*50))
...
for(i in 2:length(treatment_IDs)){
