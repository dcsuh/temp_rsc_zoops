...
m1_sim <- function(R, time, f, u, length, gamma, Z){
  ...
  end_data <- slice_max(output, time)[,3]  
  }
...
m1_ll <- function(f, u){
  data <- tmp
  R_end <- as.data.frame(mapply(m1_sim, R=data$amt_init, time=data$time/60/24, f=f/data$vol,
...
#  exp_data <- dataset_tmp
  ...
  I_end <- as.data.frame(mapply(m1_sim, R=exp_data$resource, time=exp_data$time, f=f/50, u=u, length = exp_data$life_mm, gamma=2, Z=200*50))
...
start_time <- Sys.time()
tmp <- data
m1_u_f_fit <- mle2(m1_ll, start=list(u = 0.0001, f=15), 
                     control=list(parscale = c(u = 0.0001, f=15)),
...
data_summ %>% ggplot(.,aes(x=resource, y=model_prev, group=temp, color=as.factor(temp))) + geom_point()
