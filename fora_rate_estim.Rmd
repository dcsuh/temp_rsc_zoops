...
```{r, m1_ll}
m1_ll <- function(f){
  nll <- tibble(nll=0, data$ID)
  count <- 0
    for (i in 1:length(unique(data$treatment_ID))) {
    sub_data <- data[data$treatment_ID==treatment_IDs[i], ]
    maxTime <- 12
    times <- seq(0, maxTime, by=0.1)
    params <- c(f=f,
                length=sub_data$mm,
    ...
    output <- as.data.frame(lsoda(xstart, times, m1_num_sol, params))
      ...
      for (j in 1:length(sub_data$ID)){
      tmp <- output %>% filter(time<=(sub_data$time/60))
      r_end <- slice_max(tmp, time)[,2]
      
      r_resid <- r_end - sub_data$amt_rem
      
      count <- count+1  
      nll$nll[count] <- dnorm(r_resid, mean = 0, sd = sd(r_resid, na.rm = T), log = T)
      }
    }
