...
get_arr <- function(data){
  ...
  dat <- c()
  for (i in 2:3){
      dat[i-1] <- ((log(data$rate_mean[i]/f_ref))/((1/15)-(1/data$temp[i])))
  }
  return(mean(dat))
}
```
...
```{r}
dat %>% filter(R==1 & f>0) %>% ggplot(., aes(x = arr_t, y = temp, fill = 1-end/R)) + geom_tile() + facet_wrap(~f) + scale_fill_viridis_c()
```
...
```{r, m2_f_fit for each resource level}
start_time <- Sys.time()
m2_f_fits <- list()
...
<!-- $$ -->
<!-- \frac{dR}{dt} = -L^{\gamma} \frac{R}{(1+ fhR)^2} \\ -->
<!-- $$ -->
...
```{r}
dat %>% na.omit() %>% ggplot(., aes(x = h, y = R, fill = 1-end/R)) + geom_tile() + facet_wrap(~f) + scale_fill_viridis_c()
```



$$
observed\; f = - \frac{fL^{\gamma}RS}{1 + fL^{\gamma}hR} \\
1 + fL^{\gamma}hR\;observed\; f = - fL^{\gamma}RS \\
fL^{\gamma}hR\;observed\; f = - fL^{\gamma}RS -1 \\
h = - \frac{fL^{\gamma}RS - 1}{fL^{\gamma}R\;observed\; f}\\
$$
...
dat %>% filter(h == param_set$h[x] & arr_t == param_set$arr_t[y] & f > 0 & R > 0) %>% 
...
start_time <- Sys.time()
m4_f_fit <- mle2(m4_ll,
                start=list(f=0.001, arr_t=100, h=10),
                method = "Nelder-Mead")
end_time <- Sys.time()
...
```{r, eval = F}
# Define function for R(t)
Rt <- function(t, R0, a, h) {
  K <- exp(-log(1 + a * h * R0))
  R <- K * exp(-log(1 + a * h * R0) + a * h * t)
  return(R)
}
...
```{r, eval = F}
m3_ll_alt <- function(f, h){
  resid_df <- tibble(resid=0, data$ID)
  for (i in 1:nrow(data)) {
    maxTime <- data$time[i]
    
    r_end <- Rt(maxTime, data$resource_tot[i], f, h)
...
```{r, eval = F}
# Define function for R(t) with allometric scaling
RtWithAllometricScaling <- function(t, R0, a, h, consumerBodyMass) {
  aPrime <- a * (consumerBodyMass)^(0.67)
  K <- exp(-ln(1 + aPrime * h * R0))
  R <- K * exp(-ln(1 + aPrime * h * R0) + aPrime * h * t)
  return(R)
}
```
