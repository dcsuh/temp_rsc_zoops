...
```{r}
m0_params <- data_summ %>% select(temp, resource, rate_mean, conc_mean, amt_mean, time_mean, length_mean, ID)
...
m0_ll <- function(f){
  ...
  resid_df <- tibble(resid=0, data$ID)
    ...
    resid_df$resid[i] <- r_end - data$amt_rem[i] #amt_rem_tot is total R remaining instead of conc.
    }
...
```{r}
#resid_df %>% ggplot(.,aes(x=resid)) + geom_histogram(binwidth=0.01)
```
