...
Let's try to write out an analytical solution

$$
f = -ln(\frac{R_t}{R_0}) \frac{V}{tL^\gamma} \frac{1}{e^{T_A(\frac{1}{T_R}-\frac{1}{T})}}
$$

```{r}
arr_t <- 1000
data_summ %<>% mutate(m2_f = -log(conc_mean/resource)*(15/(time_mean*mm_mean^2))*(1/(exp(arr_t*((1/15)-(1/temp))))))
```


I need to rearrange that equation to get Rt on one side
...
```{r, m2_ll}
kelvin <- 0 #set to 273 if you want it in Kelvin
...
summary(readRDS(here("mle", "m2_f_fits.rds")))
```
...
start_time <- Sys.time()
m4_f_fit <- mle2(m4_ll,
                start=list(f=0.003, arr_t=100, h=10),
                method = "L-BFGS-B",
                lower = c(0, 0, 0),
                upper = c(Inf, Inf, Inf))
end_time <- Sys.time()
