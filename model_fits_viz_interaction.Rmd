...
seq_plot_temp <- function(rate){
  seq_data %>% na.omit() %>% filter(resource = c(0.1, 0.5, 1))
  ggplot(.) +
...
data_summ %<>% mutate(m6b_rate = mapply(get_rate, f=m6b_f, arr=m6b_arr, h=m6b_h*vol + m6b_w*data_summ$temp, temp=data_summ$temp, resource=data_summ$resource))
  seq_data %<>% mutate(m6b_rate = mapply(get_rate, f=m5c_f, arr=m6b_arr, h=m6b_h*vol + m6b_w*seq_data$temp, temp=seq_data$temp, resource=seq_data$resource))
...
m6_range %<>% mutate(h_b = m6b_h*vol + m6b_w*temp,
                     h_c = m6c_h*vol*exp(m6c_w*temp))
...
m7_h_range <- m7_range %>% mutate(h_b = m7b_h*vol + m7b_w*temp,
                               h_c = m7c_h*vol*exp(m7c_w*temp),
                               h_d = m7d_h*vol*exp(m7d_w*temp),
                               h_e = m7e_h*vol + m7e_w*temp)
```
