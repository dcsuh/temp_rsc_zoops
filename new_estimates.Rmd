...
m5b_ll <- function(f, arr_t, h, p){
  ...
  data <- tmp
  output <- as.data.frame(mapply(m4_sim, R=data$amt_init, time=data$time/60/24, f=f/data$vol,
                                 length=data$mm, gamma=2, arr_t=data$resource*p + arr_t, ref_t=15, temp=data$temp, h=h*data$vol))
  colnames(output) <- "endpoint"
  data$end <- output$endpoint
  data %<>% mutate(resid = log(end) - log(amt_rem))
...
m5c_ll <- function(f, arr_t, h, p){
  ...
  data <- tmp
  output <- as.data.frame(mapply(m4_sim, R=data$amt_init, time=data$time/60/24, f=f/data$vol,
                                 length=data$mm, gamma=2, arr_t=p^data$resource + arr_t, ref_t=15, temp=data$temp, h=h*data$vol))
  colnames(output) <- "endpoint"
  data$end <- output$endpoint
  data %<>% mutate(resid = log(end) - log(amt_rem))
