...
m3_ll <- function(f, h){
  data <- tmp
  output <- as.data.frame(mapply(m3_sim, R=data$resource, time=data$time/60/24, f=f/data$vol,
...
m3a_ll <- function(f, k){
  data <- tmp
  output <- as.data.frame(mapply(m3a_sim, R=data$resource, time=data$time/60/24, f=f/data$vol,
...
m4a_ll <- function(f, arr_t, k){
  data <- tmp
  output <- as.data.frame(mapply(m4a_sim, R=data$resource, time=data$time/60/24, f=f/data$vol,
...
m5_ll <- function(f, arr_t_0.1, arr_t_0.5, arr_t_1, h){
  ...
  output <- as.data.frame(mapply(m4_sim, R=data$resource, time=data$time/60/24, f=f/data$vol,
