...
```{r}
f_seq %<>% mutate(amt_cons_mean = (resource-amt_rem_mean)*15/1000,
                  amt_cons_se = amt_rem_se*15/1000)
```
...
seq_data %>% filter(temp %in% const_temp) %>% 
  ...
  geom_point(aes(x=resource, 
             ...
             size=3) + 
    geom_linerange(aes(x=resource, 
                       ymin=amt_cons_mean-amt_cons_se,
                       ymax=amt_cons_mean+amt_cons_se,
                       color=as.factor(temp))) +
  labs(x = "Resource (mgC/L)", 
...
seq_data %>% filter(temp %in% const_temp) %>% 
  ...
  geom_line(size=1) + 
  geom_point(aes(x=resource, 
                  y=amt_cons_mean/15*1000, 
                  group = as.factor(temp), 
                  color = as.factor(temp)),
             size=3) + 
    geom_linerange(aes(x=resource, 
                       ymin=amt_cons_mean/15*1000-amt_cons_se/15*1000,
                       ymax=amt_cons_mean/15*1000+amt_cons_se/15*1000,
                       color=as.factor(temp))) +
  labs(x = "Resource (mgC/L)", 
       y = "Algae consumed (mgC)",
       color = "Temp") + 
