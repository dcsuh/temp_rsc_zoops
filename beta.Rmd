...
AICtab(mod_list, logLik=T, base=T)
```


```{r}

beta_summ %<>% mutate(beta_m1 = f*coef(m1_u_fit)[1],
                      beta_m2 = f*coef(m2_u_fit)[1]*exp(coef(m2_u_fit)[2]*(1/ref_t-1/temp)),
                      beta_m3 = f*(coef(m3_u_fit)[1]*resource/(coef(m3_u_fit)[2] + resource)),
                      beta_m4 = f*(coef(m4_u_fit)[1]*exp(coef(m4_u_fit)[2]*(1/ref_t-1/temp))*resource/(coef(m4_u_fit)[3] + resource)))

beta_summ %>% ggplot(.,aes(x=beta_m4, y=beta_m3)) + geom_point()
