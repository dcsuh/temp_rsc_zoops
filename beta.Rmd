...
dataset %<>% mutate(inf_status = replace_na(inf, 0), dead = ifelse(is.na(REMOVED) & is.na(KBP), 1, 0))
...
```{r, eval=F}
m1_u_fit <- mle2(m1_ll, start=list(u = 0.0000001), 
...
```{r}
ggplot(beta_summ,aes(x=prev, y=beta_m1)) + geom_point() 
ggplot(beta_summ,aes(x=prev, y=beta_m2)) + geom_point() 
ggplot(beta_summ,aes(x=prev, y=beta_m3)) + geom_point() 
ggplot(beta_summ,aes(x=prev, y=beta_m4)) + geom_point() 
```
...
```{r}
prob_inf <- dataset %>% group_by(trt) %>% summarize(n = n(),
                                                    n_inf = sum(inf_status),
                                                    prob = n_inf/n)
```

```{r}
beta_summ %<>% left_join(.,prob_inf, by="trt")
```

fix this
```{r, eval=F}
beta_summ %<>% mutate(m1_prev = dnorm(n, size = 1, prob=exp(-beta_m1*z)),
                      m2_prev = dnorm(n, size = 1, prob=exp(-beta_m2*z)),
                      m3_prev = dnorm(n, size = 1, prob=exp(-beta_m3*z)),
                      m4_prev = dnorm(n, size = 1, prob=exp(-beta_m4*z)))
```
