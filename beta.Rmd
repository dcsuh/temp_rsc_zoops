...
m2_ll <- function(u, arr_t){
  tmp <- dataset
    ...
    dbinom(x = inf_status, size = 1, prob=exp(-f*u*spore_exposure*time*exp(arr_t*(1/ref_t-1/temp))), log = T)
  }
...
m3_ll <- function(u, k){
  tmp <- dataset
    ...
    dbinom(x = inf_status, size = 1, prob = exp(-f*(u*resource/(k + resource))*spore_exposure*time), log = T)
  }
...
m4_ll <- function(u, arr_t, k){
  tmp <- dataset
    ...
    dbinom(x = inf_status, size = 1, prob = exp(-f*(u*exp(arr_t*(1/ref_t-1/temp))*resource/(k + resource))*spore_exposure*time), log = T)
  }
