...
data_summ %<>% mutate(m3_rate = ((m3_f)/(1+m3_f*(mm_mean^gamma)*m3_h*resource)))

seq_data %<>% mutate(m3_rate=((m3_f)/(1+m3_f*(mean_length^gamma)*m3_h*resource)))
```
...
$$
observed\; f = \frac{fe^{T_{A}(1/T_{R}-1/T)}}{1 + fe^{T_{A}(1/T_{R}-1/T)}hL^{\gamma}R} \\
$$
...
seq_data %<>% mutate(m4_rate = ((m4_f*exp(m4_arr*(1/ref_t - 1/temp)))/
                                   (1+m4_f*exp(m4_arr*(1/ref_t - 1/temp))*m4_h*(mean_length^gamma)*resource)))
```
...
geom_point(data=data_summ, aes(x=resource, 
                          ...
                          color = as.factor(temp),
                          shape = as.factor(temp))) + 
labs(x="Resource", y="Foraging rate (mL/min)", color="Temperature", shape="Temperature", title = "Full model") 
```
...
```{r}
m4_sim <- function(f, R, temp, length, gamma, 
