...
```{r}
m0_plot1 <- data_summ %>% ggplot(., aes(x=as.factor(temp),
                          y=rate_len_mean,
...
m0_plot2 <- data_summ %>% ggplot(., aes(x=as.factor(resource), 
                          y=rate_len_mean, 
...
```{r}
m1_plot1 <- data_summ %>% ggplot(., aes(x=as.factor(temp),
                          y=rate_len_mean,
                          group = as.factor(resource),
                          color = as.factor(resource),
                          shape = as.factor(resource))) +
...
# geom_linerange(aes(ymin=rate_mean-rate_se,
#                  ymax=rate_mean+rate_se)) +
labs(x="Temperature", y="Foraging rate (mL/min)", color="Resource", shape="Resource", title = "Independent model with size correction") +
new_scale_color() +
geom_point(aes(x=as.factor(temp),
               y=m1_rate,
               shape=as.factor(resource))) +
...
m1_plot2 <- data_summ %>% ggplot(., aes(x=as.factor(resource), 
                          y=rate_len_mean, 
                          group = as.factor(temp), 
                          color = as.factor(temp),
                          shape = as.factor(temp))) + 
...
# geom_linerange(aes(ymin=rate_mean-rate_se, 
#                  ymax=rate_mean+rate_se)) +
labs(x="Resource", y="Foraging rate (mL/min)", color="Temperature", shape="Temperature", title = "Independent model with size correction") +
new_scale_color() +
geom_point(aes(x=as.factor(resource), 
               y=m1_rate, 
               shape=as.factor(temp))) +
...
seq_data %<>% mutate(m2_rate = (mm_mean^gamma)*m2_f*exp(m2_arr*(1/ref_t - 1/temp)))
```

```{r}
m2_plot1 <- data_summ %>% ggplot(., aes(x=as.factor(temp),
                          y=rate_len_mean,
                          group = as.factor(resource),
                          color = as.factor(resource),
                          shape = as.factor(resource))) +
...
m2_plot2 <- data_summ %>% ggplot(., aes(x=as.factor(resource), 
                          y=rate_len_mean, 
                          group = as.factor(temp), 
                          color = as.factor(temp),
                          shape = as.factor(temp))) + 
...
# geom_linerange(aes(ymin=rate_mean-rate_se, 
#                  ymax=rate_mean+rate_se)) +
labs(x="Resource", y="Foraging rate (mL/min)", color="Temperature", shape="Temperature", title = "Temperature-dependent model") +
new_scale_color() +
geom_point(aes(x=as.factor(resource), 
               y=m2_rate, 
               shape=as.factor(temp))) +
...
observed\; f = \frac{fL^{\gamma} - 1}{fL^{\gamma}Rh}\\
$$
...
data_summ %<>% mutate(m3_rate = ((m3_f*mm_mean^gamma)/(1+m3_f*(mm_mean^gamma)*m3_h*resource)))
data_summ %<>% mutate(m3_rate1 = ((m3_f*(mm_mean^gamma)-1)/(m3_f*(mm_mean^gamma)*m3_h*resource)))
data_summ %<>% mutate(m3_rate2 = ((m3_f)/(1+m3_f*m3_h*resource)))

seq_data %<>% mutate((m3_f*mm_mean^gamma)/(1+m3_f*(mm_mean^gamma)*m3_h*resource)))
```

```{r}
m3_plot1 <- data_summ %>% ggplot(., aes(x=as.factor(temp),
                          y=rate_len_mean,
                          group = as.factor(resource),
                          color = as.factor(resource),
                          shape = as.factor(resource))) +
...
# geom_linerange(aes(ymin=rate_mean-rate_se,
#                  ymax=rate_mean+rate_se)) +
labs(x="Temperature", y="Foraging rate (mL/min)", color="Resource", shape="Resource", title = "Resource-dependent model") +
new_scale_color() +
geom_point(aes(x=as.factor(temp),
               y=m3_rate,
               shape=as.factor(resource))) +
...
m3_plot2 <- data_summ %>% ggplot(., aes(x=as.factor(resource), 
                          y=rate_len_mean, 
                          group = as.factor(temp), 
                          color = as.factor(temp),
                          shape = as.factor(temp))) + 
...
$$
observed\; f = \frac{fe^{T_{A}(1/T_{R}-1/T)} L^{\gamma}}{1 + fe^{T_{A}(1/T_{R}-1/T)}hL^{\gamma}R} \\
$$
...
seq_data %<>% mutate(m4_rate = ((m4_f*exp(m4_arr*(1/ref_t - 1/temp))*(mm_mean^gamma))/
                                   (1+m4_f*exp(m4_arr*(1/ref_t - 1/temp))*m4_h*(mm_mean^gamma)*resource)))
```

```{r}
m4_plot1 <- data_summ %>% ggplot(., aes(x=as.factor(temp),
                          y=rate_len_mean,
                          group = as.factor(resource),
                          color = as.factor(resource),
                          shape = as.factor(resource))) +
...
# geom_linerange(aes(ymin=rate_mean-rate_se,
#                  ymax=rate_mean+rate_se)) +
labs(x="Temperature", y="Foraging rate (mL/min)", color="Resource", shape="Resource", title = "Temperature and resource dependent model") +
new_scale_color() +
geom_point(aes(x=as.factor(temp),
               y=m4_rate,
               shape=as.factor(resource))) +
...
m4_plot2 <- data_summ %>% ggplot(., aes(x=as.factor(resource), 
                          y=rate_len_mean, 
                          group = as.factor(temp), 
                          color = as.factor(temp),
                          shape = as.factor(temp))) + 
...
# geom_linerange(aes(ymin=rate_mean-rate_se, 
#                  ymax=rate_mean+rate_se)) +
labs(x="Resource", y="Foraging rate (mL/min)", color="Temperature", shape="Temperature", title = "Temperature and resource dependent model") +
new_scale_color() +
geom_point(aes(x=as.factor(resource), 
               y=m4_rate, 
               shape=as.factor(temp))) +
...
```{r}
for (i in 1:nrow(model_conc)){
  maxTime <- model_conc$time_mean[i]
  times <- seq(0,maxTime,by=0.01)
  
  params <- c(f=m0_f/vol)
  
  xstart <- c(R=model_conc$resource[i])
  
  output <- as.data.frame(lsoda(xstart, times, m0_num_sol, params))
  
  r_end <- slice_max(output, time)[,2]
  
  model_conc$m0_conc[i] <- r_end
}
```
