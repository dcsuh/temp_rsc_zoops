...
# geom_linerange(aes(ymin=rate_mean-rate_se,
#                  ymax=rate_mean+rate_se)) +
labs(x="Temperature", y="Foraging rate (mL/min)", color="Resource", shape="Resource", title = "Independent model without size correction")
...
```{r}
m3a_plot1 <- data_summ %>% ggplot(., aes(x=as.factor(temp),
                          y=rate_len_mean,
                          group = as.factor(resource),
                          color = as.factor(resource),
                          shape = as.factor(resource))) +
...
m3a_plot2 <- data_summ %>% ggplot(., aes(x=as.factor(resource), 
                          y=rate_len_mean, 
                          group = as.factor(temp), 
                          color = as.factor(temp),
                          shape = as.factor(temp))) + 
...
```{r}
m3a_plot <- seq_data %>% filter(temp %in% c(15)) %>%
ggplot(.) +
geom_line(aes(x=resource, 
               y=m3a_rate)) +
geom_point(data=data_summ, aes(x=resource, 
...
# geom_linerange(aes(ymin=rate_mean-rate_se,
#                  ymax=rate_mean+rate_se)) +
labs(x="Temperature", y="Foraging rate (mL/day)", color="Resource", shape="Resource", title = "Temperature and resource dependent model") +
new_scale_color() +
geom_point(aes(x=as.factor(temp),
               y=m4_rate,
               shape=as.factor(resource))) +
...
# geom_linerange(aes(ymin=rate_mean-rate_se, 
#                  ymax=rate_mean+rate_se)) +
labs(x="Resource", y="Foraging rate (mL/day)", color="Temperature", shape="Temperature", title = "Temperature and resource dependent model") +
new_scale_color() +
geom_point(aes(x=as.factor(resource), 
               y=m4_rate, 
               shape=as.factor(temp))) +
...
```{r}
m5_coef
m5_f <- as.numeric(m7_coef[1])
m5_arr_0.1 <- as.numeric(m7_coef[2])
m5_arr_0.5 <- as.numeric(m7_coef[3])
m5_arr_1 <- as.numeric(m7_coef[4])
m5_h <- as.numeric(m7_coef[5])
...
```{r}
m6_coef
m6_f <- as.numeric(m7_coef[1])
m6_arr <- as.numeric(m7_coef[2])
m6_h_15 <- as.numeric(m7_coef[3])
m6_h_20 <- as.numeric(m7_coef[4])
m6_h_25 <- as.numeric(m7_coef[5])
...
m7_coef
m7_f <- as.numeric(m7_coef[1])
m7_arr <- as.numeric(m7_coef[2:4])
m7_h <- as.numeric(m7_coef[5:7])
```


```{r}
m7_sim <- function(f, R, temp, length, gamma, 
                   arr_t_0.1, arr_t_0.5, arr_t_1, ref_t, 
                   h_15, h_20, h_25){
...
seq_data_subset %<>% mutate(m7_end = mapply(m7_sim, f=m7_f, R=seq_data_subset$resource, temp=seq_data_subset$temp,
                                     length=mean_length,
                                     gamma=gamma,
                                     arr_t_0.1=m7_arr[1], arr_t_0.5=m7_arr[2], arr_t_1=m7_arr[3],
                                     ref_t=15,
                                     h_15=m7_h[1], h_20=m7_h[2], h_25=m7_h[3]))
