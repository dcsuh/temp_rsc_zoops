...
```{r}
all_fits <- seq_data %>% dplyr::select(resource, temp, prev, m2_I_end, m3_I_end, m4_I_end, m5_I_end) %>%
  ...
  geom_point(aes(x=resource, y=m5_I_end, color = as.factor(temp)), size=4, shape = 2) + 
  geom_point(aes(x=resource, y=prev, color=as.factor(temp)), size=4) + 
  #geom_linerange(aes(ymin = conf$lower, ymax=conf$upper, color=as.factor(temp))) +
  scale_color_manual(values = c("#619CFF", "#00BA38", "#F8766D")) +
  labs(x="Resource Conc. (mgC/L)", y="Probability of Infection", color="Temp", shape="Temp", title = "", linetype="Model") 

all_fits

ggsave("prev_fits_final.png", all_fits, path=here("mle/fit_plots"))
```
...
```{r}
susc_plot <- u_plot("m5_u") + labs(title = "Per-Temperature Resource Effect")
susc_plot
ggsave("susc_plot.png", all_fits, path=here("mle/fit_plots"))
```
