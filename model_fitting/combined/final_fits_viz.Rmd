...
viz_summ %<>% mutate(m5b_I_end = mapply(m4_sim, 
                                       ...
                                       time=1, 
                                       f=m5b_f/life_vol, 
                                       u = case_when(temp == 15 ~ m5b_u*exp(m5b_rho_15*resource),
                                                     temp == 20 ~ m5b_u*exp(m5b_rho_20*resource),
                                                     temp == 25 ~ m5b_u*exp(m5b_rho_25*resource)), 
                                       length=life_mm, 
                                       gamma=gamma, 
                                       Z=200*life_vol,
                                       ref_t = ref_t,
...
data_summ %<>% mutate(m5b_rate = (m5b_f*exp(m5b_arr*(1/ref_t - 1/temp))*(mm_mean^gamma))/
                      ...
                      m5b_u = case_when(temp == 15 ~ m5b_u*exp(m5b_arr_u*(1/ref_t - 1/temp))*exp(m5b_rho_15*resource),
                                       temp == 20 ~ m5b_u*exp(m5b_arr_u*(1/ref_t - 1/temp))*exp(m5b_rho_20*resource),
                                       temp == 25 ~ m5b_u*exp(m5b_arr_u*(1/ref_t - 1/temp))*exp(m5b_rho_25*resource)))
...
seq_data %<>% mutate(spores_consumed_m5a = mapply(m1_sim_Z, 
                                         R=resource*life_vol/1000, 
                                         time=1, 
                                         f=m5a_f/life_vol, 
                                         u = 0, 
