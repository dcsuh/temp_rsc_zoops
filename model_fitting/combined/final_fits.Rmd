...
start_time <- Sys.time()
tmp <- data
m1_u_f_fit <- mle2(m1_ll, start=list(u = 0.001, f=18, arr_t_f=30, h=17000, w=-0.2),
                     control=list(parscale = c(u = 0.001, f=1, arr_t_f=10, h=10000, w=0.1)),
...
# Interaction Model
this one might be more complicated


```{r}
# m5_ll <- function(f, u, arr_t_f, h, w, rho){
#   data <- tmp
#   R_end <- as.data.frame(mapply(m3_sim, 
#                                 R=data$amt_init*fora_vol/1000, 
#                                 time=data$time/60/24, 
...
# m5_u_f_fit[1] <- mle2(m4_ll, start=list(u = 0.0001, 
#                                         f=1, 
#                                         arr_t_f=10, 
#                                         h=1,
#                                         w=0.1,
...
#                                                rho=0.1)),
#                      skip.hessian=F, method="L-BFGS-B", 
#                    lower=c(u = 0, f = 0, arr_t_f = 0, h = 0, w=-Inf, rho=-Inf), 
#                    upper=c(u = 1, f = Inf, arr_t_f = Inf, h = Inf, w=Inf, rho=Inf))
# 
# tmp <- data %>% filter(temp == temp_IDs[2])
# tmp_dataset <- dataset %>% filter(temp == temp_IDs[2])
# m5_u_f_fit[2] <- mle2(m4_ll, start=list(u = 0.0001, 
#                                         f=1, 
#                                         arr_t_f=10, 
#                                         h=1,
#                                         w=0.1,
...
# m5_u_f_fit[3] <- mle2(m4_ll, start=list(u = 0.0001, 
#                                         f=1, 
#                                         arr_t_f=10, 
#                                         h=1,
#                                         w=0.1,
...
data_summ %<>% mutate(m1_rate = (m1_f*exp(m1_arr*(1/ref_t - 1/temp))*(mm_mean^gamma))/
                        (1+m1_f*exp(m1_arr*(1/ref_t - 1/temp))*(mm_mean^gamma)*m1_h*exp(m1_w*temp)*resource*fora_vol/1000),
                      m1_u = m1_u)
```
...
m2_f <- as.numeric(m2_coef[2])
m2_arr <- as.numeric(m2_coef[3])
m2_arr_u <- as.numeric(m2_coef[4])
m2_h <- as.numeric(m2_coef[4])
m2_w <- as.numeric(m2_coef[4])


data_summ %<>% mutate(m2_rate = (m2_f*exp(m2_arr*(1/ref_t - 1/temp))*(mm_mean^gamma))/
                        (1+m2_f*exp(m2_arr*(1/ref_t - 1/temp))*(mm_mean^gamma)*m2_h*exp(m2_w*temp)*resource*fora_vol/1000),
...
data_summ %<>% mutate(m3_rate = (m3_f*exp(m3_arr*(1/ref_t - 1/temp))*(mm_mean^gamma))/
                        (1+m3_f*exp(m2_arr*(1/ref_t - 1/temp))*(mm_mean^gamma)*m3_h*exp(m3_w*temp)*resource*fora_vol/1000),
...
data_summ %<>% mutate(m4_rate = (m4_f*exp(m4_arr*(1/ref_t - 1/temp))*(mm_mean^gamma))/
                        (1+m4_f*exp(m4_arr*(1/ref_t - 1/temp))*(mm_mean^gamma)*m4_h*exp(m4_w*temp)*resource*fora_vol/1000),
...
m5_u_15 <- as.numeric(m5_coef[1])
m5_u_20 <- as.numeric(m5_coef[2])
m5_u_25 <- as.numeric(m5_coef[3])
m5_f <- as.numeric(m5_coef[4])
...
data_summ %<>% mutate(m5_rate = (m5_f*exp(m5_arr*(1/ref_t - 1/temp))*(mm_mean^gamma))/
                        (1+m5_f*exp(m5_arr*(1/ref_t - 1/temp))*(mm_mean^gamma)*m5_h*exp(m5_w*temp)*resource*fora_vol/1000),
                      m5_u = case_when(temp == 15 ~ m5_u_15*exp(m5_rho_15*temp),
                                       temp == 20 ~ m5_u_20*exp(m5_rho_20*temp),
                                       temp == 25 ~ m5_u_25*exp(m5_rho_25*temp)))
