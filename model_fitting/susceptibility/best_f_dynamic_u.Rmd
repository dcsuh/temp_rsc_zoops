...
m2_sim <- function(R, time, f, u, length, gamma, Z, ref_t, arr_t_f, arr_t_u, h, temp){
  ...
  output <- as.data.frame(lsoda(y=xstart, times, m1_num_sol, params))
  #output <- as.data.frame(lsoda(y=xstart, times, m1_num_sol_alt, params)) #defined in best_f_static_u.Rmd
...
```{r}
# exp_data %<>% mutate(Z_end = mapply(m2_sim, 
#                                 R=exp_data$resource*life_vol/1000, 
#                                 time=exp_data$time, 
#                                 f=f_est/life_vol, 
#                                 u=u, 
...
m3u_ll <- function(u){
  ...
  exp_data <- iter_data %>% filter(exposed==TRUE)
...
for(i in 1:length(resource_IDs)) {
  ...
  iter_data <- dataset %>% filter(resource == resource_IDs[i])
  m3_u_fits[i] <- mle2(m3u_ll, start=list(u = u_vec[i]), 
...
saveRDS(m3_u_fits, file = here("mle","susc","m3_u_fits.rds"))
```
...
output %>% ggplot(.,aes(x=time, y=R)) + geom_point()
output %>% ggplot(.,aes(x=time, y=S)) + geom_point() + ylim(0,1)
output %>% ggplot(.,aes(x=time, y=I)) + geom_point() + ylim(0,1)
output %>% ggplot(.,aes(x=time, y=Z)) + geom_point()

# dat1 <- tibble(res = seq(0.1,1,by=0.1))
# dat1 %<>% mutate(res = res/1000,
#                  f = (f*exp(arr_t_f*(1/ref_t - 1/temp))*(length^gamma)*res)/
#                 (1+f*exp(arr_t_f*(1/ref_t - 1/temp))*(length^gamma)*h*res),
#                 f_pc = (f*exp(arr_t_f*(1/ref_t - 1/temp))*(length^gamma))/
#                 (1+f*exp(arr_t_f*(1/ref_t - 1/temp))*(length^gamma)*h*res))
# dat1 %>% ggplot(., aes(x=res, y=f)) + geom_point()
...
m3_u_exp_per_temp_fits <- list()

tmp_dataset <- dataset %>% filter(temp == temp_IDs[1])

m3_u_exp_per_temp_fits[[1]] <- mle2(m3_rho_ll, start=list(u = 0.000001,
                                   rho = 1), 
                     control=list(parscale = c(u = 0.000001,
                                   rho = 1)),
                     skip.hessian=F, method="L-BFGS-B", 
...
m3_u_exp_per_temp_fits[[2]] <- mle2(m3_rho_ll, start=list(u = 0.000001,
                                   rho = 1), 
                     control=list(parscale = c(u = 0.000001,
                                   rho = 1)),
                     skip.hessian=F, method="L-BFGS-B", 
...
m3_u_exp_per_temp_fits[[3]] <- mle2(m3_rho_ll, start=list(u = 0.001,
                                   rho = 1), 
                     control=list(parscale = c(u = 0.001,
                                   rho = 1)),
                     skip.hessian=F, method="Nelder-Mead")
...
coef(m3_u_exp_per_temp_fits[[1]])
coef(m3_u_exp_per_temp_fits[[2]])
coef(m3_u_exp_per_temp_fits[[3]])

saveRDS(m3_u_exp_per_temp_fits, file = here("mle","susc","m3_u_exp_per_temp_fits.rds"))
```
...
```{r}
m5a_per_ll <- function(u, arr_u, rho_15, rho_20, rho_25){
  ...
  for(j in 1:length(resource_IDs)){  
      ...
      exp_data <- dataset %>% filter(exposed==TRUE)
      exp_data %<>% filter(temp == temp_IDs[j])
...
tmp <- data
m5a_per_fit <- mle2(m5a_per_ll, start=list(u = 0.0001, 
                                     arr_u = 10,
                                     rho_15 = 0.1,
                                     rho_20 = 0.1,
                                     rho_25 = 0.1), 
...
saveRDS(m5a_per_fit, file = here("mle","susc","m5a_per_fit.rds"))
```
...
m3_u_fits <- readRDS(file = here("mle", "susc", "m3_u_fits.rds"))
...
m5a_per_fit <- readRDS(file = here("mle", "susc", "m5a_per_fit.rds"))
...
getAIC(m3_u_per_fit)
getAIC(m3_u_exp_fit)
#getAIC(m3_u_exp_per_temp_fits[[1]]) + getAIC(m3_u_exp_per_temp_fits[[2]]) + getAIC(m3_u_exp_per_temp_fits[[3]])
getAIC(m4_u_fit)
# getAIC(m5a_u_fit)
# getAIC(m5b_u_fit)
getAIC(m5a_per_fit)
...
```{r}
m3_u_low <- coef(m3_u_fits[[1]])
m3_u_med <- coef(m3_u_fits[[2]])
m3_u_high <- coef(m3_u_fits[[3]])
...
```{r}
m3_u_15 <- coef(m3_u_exp_per_temp_fits[[1]])[1]
m3_rho_15 <- coef(m3_u_exp_per_temp_fits[[1]])[2]
m3_u_20 <- coef(m3_u_exp_per_temp_fits[[2]])[1]
m3_rho_20 <- coef(m3_u_exp_per_temp_fits[[2]])[2]
m3_u_25 <- coef(m3_u_exp_per_temp_fits[[3]])[1]
m3_rho_25 <- coef(m3_u_exp_per_temp_fits[[3]])[2]

viz_summ %<>% mutate(m3_per_I_end = mapply(m3_sim, 
                                    R=resource*life_vol/1000, 
                                    time=time_mean, 
                                    f=f_est/life_vol, 
                                    u=case_when(temp==15 ~ m3_u_15*exp(m3_rho_15*resource),
                                                temp==20 ~ m3_u_20*exp(m3_rho_20*resource),
                                                temp==25 ~ m3_u_25*exp(m3_rho_25*resource)),
                                    length = mm_mean, 
                                    gamma=gamma, 
                                    Z=spore_conc*life_vol,
                                    ref_t = ref_t,
...
viz_summ %<>% mutate(m3_I_end = mapply(m3_sim, 
                                    ...
                                    time=time_mean, 
                                    f=f_est/life_vol, 
                                    u=m3_u*exp(m3_rho*resource/1000), 
                                    length = mm_mean, 
                                    gamma=gamma, 
                                    Z=spore_conc*life_vol,
                                    ref_t = ref_t,
