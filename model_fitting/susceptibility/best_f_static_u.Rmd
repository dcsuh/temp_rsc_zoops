...
# Notes
 ...
 - it seems that whatever model specification we use needs to be able to handle the fact that prevalence was so much higher at low temp, high resource. This flips the order of u at high resource and makes it difficult for general models to fit. We don't have a way to incorporate temperature as a function that will allow this to occur. It needs to be categorical. Because of this, the best model fits the resource-dependent effect on a per-temperature basis. this results in a model that is nearly as good a fit as the per-treatment version
...
we'll use this version every time we get u per treatment
```{r}
u_per_ll <- function(u){
...
for(i in 1:length(treatment_IDs)){
  ...
  iter_data <- exp_data %>% filter(treatment == treatment_IDs[i])
  u_fits[[i]] <- mle2(u_per_ll, start=list(u = u_vec[i]), 
                     control=list(parscale = c(u = u_vec[i])),
                     skip.hessian=F, method="Brent",
                     lower = c(u = 0),
                     upper = c(u = 1/max(iter_data$spores_consumed)))
...
u_per <- tibble(ID = treatment_IDs,
                u = c(NA))

for(i in 1:length(treatment_IDs)){
 print(coef(u_fits[[i]]))
 u_per$u[i] <- coef(u_fits[[i]])
 tmp_AIC <- getAIC(u_fits[[i]])
 AIC <- tmp_AIC + AIC
}

for(i in 1:length(treatment_IDs)){
  iter_data <- exp_data %>% filter(treatment == treatment_IDs[i])
  print(paste(i, 1/max(iter_data$spores_consumed)))
}

u_per <- data_summ %>% dplyr::select(temp, resource, ID) %>% left_join(., u_per)

saveRDS(u_fits, file = here("mle","susc","u_per_fits_alt.rds"))

```

```{r}
ggplot(u_per, aes(x=resource, y=u, color=as.factor(temp))) + geom_point()

ggplot(u_per, aes(x=temp, y=u, color=as.factor(resource))) + geom_point()
```
...
# check this!!!
Note: results were robust to method. the commented out methods also provided the same results
strangely, when doing it using this method, we get a non-monotonic relationship across temperature. This may not be that strange though because we see such different results across resource conditions. When conflating resource, it is at 20C we get the highest per-spore susceptibility, followed by 15C and then 25C.
...
exp(arr_t_u*(1/ref_t - 1/15))
exp(arr_t_u*(1/ref_t - 1/20))
exp(arr_t_u*(1/ref_t - 1/25))
...
m2_u_fit <- mle2(m2_ll, start=list(u = 0.00035,
                                   arr_t_u = 63),
...
#for(i in 1:length(resource_IDs))
  ...
  iter_data <- exp_data %>% filter(resource == resource_IDs[1])
  m3_u_fits[1] <- mle2(u_per_ll, start=list(u = u_vec[1]), 
                     control=list(parscale = c(u = u_vec[1])),
                     skip.hessian=F, method="L-BFGS-B"
                     ,
                     lower=c(u = 0),
  ...
  iter_data <- exp_data %>% filter(resource == resource_IDs[2])
  m3_u_fits[2] <- mle2(u_per_ll, start=list(u = u_vec[2]),
                   control=list(parscale = c(u = u_vec[2])),
                   skip.hessian=F, method="L-BFGS-B"
                   ,
                   lower=c(u = 0),
  ...
  iter_data <- exp_data %>% filter(resource == resource_IDs[3])
  m3_u_fits[3] <- mle2(u_per_ll, start=list(u = u_vec[3]), 
                 control=list(parscale = c(u = u_vec[3])),
                 skip.hessian=F, method="BFGS")
