...
```{r}
# m1_sim_list <- function(R, time, f, u, length, gamma, Z, ref_t, arr_t_f, arr_t_u, h, temp){
#   times <- seq(0, time, by=0.01)
#   xstart <- c(R=R,
#               S=1,
#               I=0,
...
#   output_list[[i]] <- m1_sim_list(R = data_summ$temp[i]*life_vol/1000,
#                                   time=data_summ$time_mean[i],
#                                   f=f_est/life_vol,
#                                   u=0.0001,
#                                   length = data_summ$mm_mean[i],
#                                   gamma=gamma,
#                                   Z=spore_conc*life_vol,
#                                   ref_t = ref_t,
#                                   arr_t_f = arr_f_est,
#                                   h = h_est*exp(data_summ$temp[i]*w_est),
#                                   temp=data_summ$temp[i])
# }
...
# tmp <- output_list[[i]]
# 
# output_plot[[i]] <- tmp %>% ggplot(., aes(x=time, y=Z)) + geom_point()
# }
```
...
```{r}
# (f*exp(arr_t*(1/ref_t - 1/temp))*(length^gamma)*R)/(1+f*exp(arr_t*(1/ref_t - 1/temp))*(length^gamma)*h*R)
```
...
#   output <- as.data.frame(lsoda(y=xstart, times, m1_num_sol, params))
#   
#   output %<>% mutate(f_R = (f/life_vol*exp(arr_t_f*(1/ref_t - 1/temp))*(length^gamma))/
#                 (1+f*exp(arr_t_f*(1/ref_t - 1/temp))*(length^gamma)*h*R),
#                 Z_R = -f_R*Z)
```
...
#   output %>% ggplot(., aes(x=time, y=S)) + geom_point()
#   output %>% ggplot(., aes(x=time, y=I)) + geom_point()
...
m1_ll <- function(u){
  ...
  exp_data %<>% mutate(I_end = spores_consumed*u*life_mm^gamma)
...
Not sure why this provides different results than using dbinom per tube but it does as of now

# ```{r}
# life_summ <- exp_data %>% group_by(treatment, spores_consumed) %>% summarize(n_inf = sum(inf),
#                                                             n_total = n())
# ```
# 
# ```{r, m1_ll}
# m1_ll <- function(u){
# 
#   life_summ %<>% mutate(I_end = spores_consumed*u)
#   
#   life_summ %<>% mutate(I_end = ifelse(I_end>1, 1, I_end))
#   
#     inf_out_alt <- function(n_inf, n_total, I_end){
#     dbinom(x = n_inf, size = n_total, prob=I_end, log=T)
#   }
#   
#   life_summ %<>% 
#     mutate(ll = mapply(inf_out_alt, n_inf = n_inf, n_total = n_total, I_end=I_end))
#   nll_sum <- -sum(life_summ$ll)
#   
#   print(u)
#   
#   return(nll_sum)
...
test different starting params. We do get different results for these
```{r}
# m2_u_fit <- mle2(m2_ll, start=list(u = 0.00001,
#                                    arr_t_u = 10), 
#                      control=list(parscale = c(u = 0.00001,
...
life_summ %<>% mutate(rate = (f_est/life_vol*exp(arr_t_f*(1/ref_t - 1/temp))*(life_mm^gamma))/
