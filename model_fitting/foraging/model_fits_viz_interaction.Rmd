...
```{r}
all_fits <- seq_data %>% dplyr::select(resource, temp_factor, m2_rate, m3_rate, m4_rate, m6c_rate) %>%
  filter(temp_factor %in% c(15, 20, 25)) %>%
...
```{r}
all_fits <- seq_data %>% dplyr::select(resource, temp_factor, amt_rem, m2_R_end, m3_R_end, m4_R_end, m6c_R_end) %>%
  filter(temp_factor %in% c(15, 20, 25)) %>%
  mutate("temp-only" = m2_R_end, "resource-only" = m3_R_end,"additive" = m4_R_end, "interactive" = m6c_R_end) %>%
  dplyr::select(-c(m2_R_end, m3_R_end, m4_R_end, m6c_R_end)) %>%
  pivot_longer(cols=c('temp-only', 'resource-only', 'additive', 'interactive')) %>%
  ggplot(.,aes(x=resource, y=value*1000/15, group = interaction(name, temp_factor),color=temp_factor)) +
  geom_line(aes(linetype=name), size = 0.75) +
  scale_linetype_manual(values = c(5, 1, 3, 6)) +
