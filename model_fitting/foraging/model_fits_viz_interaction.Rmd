...
data_summ %<>% mutate(m3_rate = ((m3_f)/(1+m3_f*m3_h*resource/1000)))

seq_data %<>% mutate(m3_rate=((m3_f)/(1+m3_f*m3_h*resource/1000)))
```
...
get_rate <- function(f, arr, temp, h, resource){
  ((f*exp(arr*(1/ref_t - 1/temp)))/
     (1+f*exp(arr*(1/ref_t - 1/temp))*h*resource))
}
```
...
data_summ %<>% mutate(m4_rate = mapply(get_rate, f=m4_f, arr=m4_arr, temp=data_summ$temp, h=m4_h, resource=data_summ$resource/1000))

seq_data %<>% mutate(m4_rate = mapply(get_rate, f=m4_f, arr=m4_arr, temp=seq_data$temp, h=m4_h, resource=seq_data$resource/1000))
```
...
data_summ %<>% mutate(m6b_rate = mapply(get_rate, f=m6b_f, arr=m6b_arr, h=(m6b_h + m6b_w*data_summ$temp), temp=data_summ$temp, resource=data_summ$resource/1000))
  seq_data %<>% mutate(m6b_rate = mapply(get_rate, f=m6b_f, arr=m6b_arr, h=(m6b_h + m6b_w*seq_data$temp), temp=seq_data$temp, resource=seq_data$resource/1000))
...
m6c_w <- as.numeric(m6c_coef[4])
  ...
  data_summ %<>% mutate(m6c_rate = mapply(get_rate, f=m6c_f, arr=m6c_arr, h=m6c_h*exp(m6c_w*data_summ$temp), temp=data_summ$temp, resource=data_summ$resource/1000))
  seq_data %<>% mutate(m6c_rate = mapply(get_rate, f=m6c_f, arr=m6c_arr, h=m6c_h*exp(m6c_w*seq_data$temp), temp=seq_data$temp, resource=seq_data$resource/1000))
...
```{r}
all_fits <- seq_data %>% dplyr::select(resource, temp_factor, m1_rate, m2_rate, m3_rate, m4_rate, m6c_rate) %>%
  filter(temp_factor %in% c(15, 20, 25)) %>%
  mutate("size" = m1_rate, "temp" = m2_rate, "res" = m3_rate, "add" = m4_rate, "inter" = m6c_rate) %>%
  dplyr::select(-c(m1_rate, m2_rate, m3_rate, m4_rate, m6c_rate)) %>%
  pivot_longer(cols=c('size', 'temp', 'res', 'add', 'inter')) %>%
  ggplot(.,aes(x=resource, y=value, group = interaction(name, temp_factor),color=temp_factor)) +
  geom_line(aes(linetype=name)) +
  ...
  geom_point(data=data_summ, aes(x=resource, 
                            ...
                            color = as.factor(temp),
                            shape = as.factor(temp))) + 
  labs(x="Resource", y="Foraging rate (mL/day)", color="Temperature", shape="Temperature", title = "", linetype="Model") 

all_fits

ggsave("all_f_fits.png", all_fits, path=here("mle/fit_plots"))
```


```{r}
all_fits <- seq_data %>% dplyr::select(resource, temp_factor, m2_rate, m4_rate, m6c_rate) %>%
  filter(temp_factor %in% c(15, 20, 25)) %>%
  ...
  geom_point(data=data_summ, aes(x=resource, 
             ...
             size = 4) + 
  scale_color_manual(values = c("#619CFF", "#00BA38", "#F8766D")) +
  labs(x="Resource Conc. (mgC/L)", y="Foraging rate (mL/day)", color="Temp", shape="Temp", title = "", linetype="Model") 
...
h_plot <- m6_range %>% ggplot(., aes(x=temp, y=h_c)) + 
  geom_line(size=2) +
  labs(x = "Temp (C)", y = "Handling time (days/mgC)", title = "Temperature effect on handling time")
