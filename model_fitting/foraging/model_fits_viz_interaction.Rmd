...
```{r}
resource_min <- round(min(data$amt_init), digits = 2)
resource_max <- round(max(data$amt_init), digits = 2)

temp_range <- seq(15,25,by=0.1)
resource_range <- seq(resource_min, resource_max,by=0.01)
...
for (i in 1:length(temp_range)){
  tmp <- tibble(temp = temp_range[i],
                resource = seq(resource_min, resource_max,by=0.01))
...
# 
# data_summ %<>% mutate(m5b_rate = mapply(get_rate, f=m5b_f, arr=data_summ$resource*m5b_p + m5b_arr, h=m5b_h, temp=data_summ$temp, resource=data_summ$resource, length = mm_mean))
#   seq_data %<>% mutate(m5b_rate = mapply(get_rate, f=m5b_f, arr=seq_data$resource*m5b_p + m5b_arr, h=m5b_h, temp=seq_data$temp, resource=seq_data$resource, length = length))
#   
# m5c_coef
...
# 
#   data_summ %<>% mutate(m5c_rate = mapply(get_rate, f=m5c_f, arr=m5c_arr*exp(data_summ$resource*m5c_p), h=m5c_h, temp=data_summ$temp, resource=data_summ$resource, length = mm_mean))
#   seq_data %<>% mutate(m5c_rate = mapply(get_rate, f=m5c_f, arr=m5c_arr*exp(seq_data$resource*m5c_p), h=m5c_h, temp=seq_data$temp, resource=seq_data$resource, length = length))
```
...
# 
# data_summ %<>% mutate(m6b_rate = mapply(get_rate, f=m6b_f, arr=m6b_arr, h=(m6b_h + m6b_w*data_summ$temp), temp=data_summ$temp, resource=data_summ$resource/1000, length = mm_mean))
#   seq_data %<>% mutate(m6b_rate = mapply(get_rate, f=m6b_f, arr=m6b_arr, h=(m6b_h + m6b_w*seq_data$temp), temp=seq_data$temp, resource=seq_data$resource/1000, length = length))
...
# 
#   data_summ %<>% mutate(m7b_rate = mapply(get_rate, f=m7b_f, arr=data_summ$resource*m7b_p + m7b_arr, h=m7b_h + m7b_w*data_summ$temp, temp=data_summ$temp, resource=data_summ$resource, length = mm_mean))
#   seq_data %<>% mutate(m7b_rate = mapply(get_rate, f=m7b_f, arr=seq_data$resource*m7b_p + m7b_arr, h=m7b_h + m7b_w*seq_data$temp, temp=seq_data$temp, resource=seq_data$resource, length = length))
#   
# m7c_coef
...
# 
#       data_summ %<>% mutate(m7c_rate = mapply(get_rate, f=m7c_f, arr=m7c_arr*exp(data_summ$resource*m7c_p), h=m7c_h*exp(m7c_w*data_summ$temp), temp=data_summ$temp, resource=data_summ$resource, length = mm_mean))
#   seq_data %<>% mutate(m7c_rate = mapply(get_rate, f=m7c_f, arr=m7c_arr*exp(seq_data$resource*m7c_p), h=m7c_h*exp(m7c_w*seq_data$temp), temp=seq_data$temp, resource=seq_data$resource, length = length))
#   
# m7d_coef
...
# 
#       data_summ %<>% mutate(m7d_rate = mapply(get_rate, f=m7d_f, arr=data_summ$resource*m7d_p + m7d_arr, h=m7d_h*exp(m7d_w*data_summ$temp), temp=data_summ$temp, resource=data_summ$resource, length = mm_mean))
#   seq_data %<>% mutate(m7d_rate = mapply(get_rate, f=m7d_f, arr=m7d_arr*exp(seq_data$resource*m7d_p), h=m7d_h*exp(m7d_w*seq_data$temp), temp=seq_data$temp, resource=seq_data$resource, length = length))
#   
# m7e_coef
...
# 
#       data_summ %<>% mutate(m7e_rate = mapply(get_rate, f=m7e_f, arr=m7e_arr*exp(data_summ$resource*m7e_p), h=m7e_h + m7e_w*data_summ$temp, temp=data_summ$temp, resource=data_summ$resource, length = mm_mean))
#   seq_data %<>% mutate(m7e_rate = mapply(get_rate, f=m7e_f, arr=m7e_arr*exp(seq_data$resource*m7e_p), h=m7e_h + m7e_w*seq_data$temp, temp=seq_data$temp, resource=seq_data$resource, length = length))
...
```{r}
#all_fits <- 
seq_data %>% dplyr::select(resource, temp_factor, m2_rate, m4_rate, m6c_rate) %>%
  filter(temp_factor %in% c(15, 20, 25)) %>%
  mutate("temp-only" = m2_rate, "additive" = m4_rate, "interactive" = m6c_rate) %>%
  dplyr::select(-c(m2_rate, m4_rate, m6c_rate)) %>%
  pivot_longer(cols=c('temp-only', 'additive', 'interactive')) %>%
  ggplot(.,aes(x=resource, y=value, group = interaction(name, temp_factor),color=temp_factor)) +
  ...
  geom_point(data=data_summ, aes(x=resource, 
             ...
             size = 4) + 
  scale_color_manual(values = c("#619CFF", "#00BA38", "#F8766D")) +
  labs(x="Resource Conc. (mgC/L)", y="Foraging rate (mL/day)", color="Temp", title = "", linetype="Model")
```
...
```{r}
#all_fits <- 
seq_data %>% dplyr::select(resource, temp_factor, m2_rate, m3_rate, m4_rate, m6c_rate) %>%
  filter(temp_factor %in% c(15, 20, 25)) %>%
  mutate("temp-only" = m2_rate, "resource-only" = m3_rate,"additive" = m4_rate, "interactive" = m6c_rate) %>%
  dplyr::select(-c(m2_rate, m3_rate, m4_rate, m6c_rate)) %>%
  pivot_longer(cols=c('temp-only', 'resource-only', 'additive', 'interactive')) %>%
  ggplot(.,aes(x=resource, y=value, group = interaction(name, temp_factor),color=temp_factor)) +
  geom_line(aes(linetype=name), size = 0.75) +
  scale_linetype_manual(values = c(6,1,3,5)) +
  geom_point(data=data_summ, aes(x=resource, 
                            y=rate_len_mean, 
                            group = as.factor(temp), 
                            color = as.factor(temp)),
...
```{r}
#ggsave("f_fits_final.png", all_fits, path=here("mle/fit_plots"))
```
...
```{r}
seq_data %>% dplyr::select(resource, temp, m3_rate, m4_rate, m6c_rate) %>%
  filter(resource %in% c(0.1, 0.5, 1)) %>%
  mutate("resource-only" = m3_rate,"additive" = m4_rate, "interactive" = m6c_rate) %>%
  dplyr::select(-c(m3_rate, m4_rate, m6c_rate)) %>%
  pivot_longer(cols=c('resource-only', 'additive', 'interactive')) %>%
  ggplot(.,aes(x=temp, y=value, group = interaction(name, resource),color=as.factor(resource))) +
  geom_line(aes(linetype=name), size = 0.75) +
  ...
  geom_point(data=data_summ, aes(x=temp, 
             ...
             size = 4) + 
  scale_color_manual(values = c("#CABD88", "#CA5216", "#65320D")) +
  labs(x="Temperature (C)", y="Foraging rate (mL/day)", color="Resource", title = "", linetype="Model")
```

```{r}
seq_data %>% dplyr::select(resource, temp, m2_rate, m3_rate, m4_rate, m6c_rate) %>%
  filter(resource %in% c(0.1, 0.5, 1)) %>%
  mutate("temp-only" = m2_rate, "resource-only" = m3_rate,"additive" = m4_rate, "interactive" = m6c_rate) %>%
  dplyr::select(-c(m2_rate, m3_rate, m4_rate, m6c_rate)) %>%
  pivot_longer(cols=c('temp-only', 'resource-only', 'additive', 'interactive')) %>%
...
#                      h_b = (m6b_h + m6b_w*temp),
#                      h_c = (m6c_h*exp(m6c_w*temp)))

m6_range %<>% mutate(h_c = (m6c_h*exp(m6c_w*temp)))

h_plot <- m6_range %>% ggplot(., aes(x=temp, y=h_c)) + 
  geom_line(size=2) +
  labs(x = "Temp (C)", y = "Handling time (days/mgC)", title = "")

ggsave("h_plot.png", h_plot, path=here("mle/fit_plots"))
...
```{r, eval=F}
amt_init <- data_summ %>% dplyr::select(temp, resource, amt_init_mean)
seq_data %<>% left_join(., amt_init)
seq_data %<>% 
  mutate(amt_diff = (amt_init_mean-amt_rem_mean)*15/1000,
         m1_consumed = resource*15/1000 - m1_R_end,
         m2_consumed = resource*15/1000 - m2_R_end,
         m3_consumed = resource*15/1000 - m3_R_end,
         m4_consumed = resource*15/1000 - m4_R_end,
         m6c_consumed = resource*15/1000 - m6c_R_end)
```
...
saveRDS(seq_data, file=here("model_fitting","foraging","rate_fit_alt.rds"))
```
